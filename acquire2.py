...
def fetch_user_followers_and_repos(user: str):
    ...
    while parse_followers:
        ...
        user_followers = set([r[1][1:] for r in re.findall(
            r'(link_type\:self"\shref=")(.*?)"', follower_html)])
...
def crawl_github(target: str, reparse=False, readme_cutoff=1000):
    ...
    try:
        ...
        readme_len = len(pd.read_csv(
            'readme_data_c.csv', index_col=['user_repo']))
    ...
    if os.path.exists(parsed_users_file):
        ...
        df = pd.DataFrame(
            [{'user': target, 'parsed': False}]).set_index('user')
